...
def FindB_PLSReg_plot(X_train,X_test,Y_train,Y_test,max_n,string):
  ...
  ax.plot(np.arange(1,max_n+1), score_list, marker='o')
  ax.set_xlabel("n_components")
  ax.set_ylabel('r2_score')
  ax.set_title(f'PLS_r2_vs_components, {string}')
  ax.grid(True)
  buf = BytesIO()
  fig.tight_layout()
...
def run_analysis_from_upload(file_media, max_pls_components=10):
  ...
  try:
    ...
    # Linear Regression (unscaled vs scaled)
    reg_lin = LinearRegression().fit(input_train_lin, output_train_lin)
    Linear_score = reg_lin.score(input_test_lin, output_test_lin)

    reg_lin_scaled = LinearRegression().fit(in_train_scaled, out_train_scaled)
    Linear_score_scaled = reg_lin_scaled.score(in_test_scaled, out_test_scaled)
    ...
    pls_plot_media = anvil.media.from_bytes(report_buf_pls.getvalue(), "image/png", name="PLS_plot.png")

    # Prepare numeric summary strings
    pls_summary = f"PLS R2 (chosen n={O_component}): {pls_score:.4f} (CV mean: {cv_mean:.4f})"
    linear_summary = f"Linear R2 (unscaled): {Linear_score:.4f} | (scaled): {Linear_score_scaled:.4f}"
