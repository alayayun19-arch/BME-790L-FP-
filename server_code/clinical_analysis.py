...
# # ---------- Helper: generic DM vs Non-DM domain analysis ----------
...
# ---------- helper: plot ALL selected metrics in ONE figure ----------
def _plot_all_metrics_in_one(dm_dict, non_dict):
  """
    dm_dict:  {metric: array}
    non_dict: {metric: array}

    Produces one big bar chart:
       x-axis = metrics
       bars = DM mean ± sd, Non-DM mean ± sd
    """

  metrics = list(dm_dict.keys())
  x = np.arange(len(metrics))

  dm_means = [dm_dict[m].mean() for m in metrics]
  non_means = [non_dict[m].mean() for m in metrics]

  dm_sds = [dm_dict[m].std(ddof=1) for m in metrics]
  non_sds = [non_dict[m].std(ddof=1) for m in metrics]
  ...
  ax.set_ylabel("Value")
  ax.set_title("DM vs Non-DM: Selected Brain Metrics Comparison")
  ax.legend()
  fig.tight_layout()
  ...
  return anvil.BlobMedia(
    "image/png", buf.read(), name="all_metrics_barplot.png"
  )
...
def analyze_dm_vs_non_dm(file_media):
  ...
  for col in features:
    ...
    dm_vals = pd.to_numeric(df_cmp.loc[df_cmp["Group"] == "DM", col], errors="coerce").dropna()
    non_vals = pd.to_numeric(df_cmp.loc[df_cmp["Group"] == "NON-DM", col], errors="coerce").dropna()
    ...
    # ----- 5. Build the single combined bar chart -----
  plot_features = [
    "global GM / ICV", "global WM / ICV", "global CSF / ICV",
    "whole brain-FA", "whole brain-MD", "whole brain-L1", "whole brain-RD",
    "wmh registered",
    ...
    "R postcentral gyrus-RD",
    "R angular gyrus-RD",
    "L hippocampus-RD", "R hippocampus-RD",
  ]
  ...
  for m in plot_features:
    if m not in df_cmp.columns:
      continue

    dm_vals = pd.to_numeric(df_cmp.loc[df_cmp["Group"] == "DM", m], errors="coerce").dropna()
    non_vals = pd.to_numeric(df_cmp.loc[df_cmp["Group"] == "NON-DM", m], errors="coerce").dropna()
  ...
  combined_plot = _plot_all_metrics_in_one(dm_dict, non_dict)
  ...
  return {
    ...
    "n_dm": n_dm,
    "n_non": n_non,
    "combined_plot": combined_plot,   # only ONE image
  }
